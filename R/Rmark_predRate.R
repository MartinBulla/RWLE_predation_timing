...
nest <- nest[which(nest$fatecode!=5), ]  # nests of drop unknown fate
nrow(nest) # n = 65 unknown fate
nest$Fate[nest$fatecode==0] = 1  # successful
nest$Fate[nest$fatecode==1] = 0  # depredated
nest$Fate[nest$fatecode==2] = 1  # deserted
table(nest$Fate)
...
# DSR varies by year
Year=mark(nest,nocc=occ,model="Nest",
model.parameters=list(S=list(formula=~Year)), groups="Year")

# DSR varies with nest age
# NOT RUN would need to drop NestAge = NA to run this model
# nestAge=mark(nest,nocc=occ,model="Nest",
# model.parameters=list(S=list(formula=~NestAge)))

# DSR follows a linear trend through time
TimeTrend=mark(nest,nocc=occ,model="Nest",
model.parameters=list(S=list(formula=~Time)))

# DSR follows a linear trend through time, same slope but different intercepts
# additive model for year + trend
YearPlusTimeTrend=mark(nest,nocc=occ,model="Nest",
model.parameters=list(S=list(formula=~Year+Time)), groups="Year")

# DSR follows a linear trend through time, different slopes and intercepts among years
# additive model for year * trend
YearByTimeTrend=mark(nest,nocc=occ,model="Nest",
model.parameters=list(S=list(formula=~Year*Time)), groups="Year")
...
# DSR varies with year + quadratic trend through time
YearPlusTimeQuad=mark(nest,nocc=occ,model="Nest",
                       model.parameters=list(S=list(formula=~Year+(Time+I(Time^2)))), 
                       groups="Year")

# DSR varies with year * quadratic trend through time
# need brackets around (time+time2) so you get interaction of year*quad
# parameter count should be 3 years x 3 quadratic terms for K = 9
YearByTimeQuad=mark(nest,nocc=occ,model="Nest",
                     model.parameters=list(S=list(formula=~Year*(Time+I(Time^2)))), 
                     groups="Year")
...
sink("Data/results.table.txt") # capture screen output to file
